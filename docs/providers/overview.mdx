...
description: "Provider 是 Keep 的核心组件，使其能够与第三方产品交互。它通过可扩展的 Python 代码实现，方便增强和定制。"
---

Provider 是 Keep 的核心组件，使 Keep 能够查询数据、发送通知、获取告警或管理第三方工具。

这些第三方工具包括但不限于：Datadog、Cloudwatch 和 Sentry（用于数据查询和/或告警管理），以及 Slack、Resend、Twilio 和 PagerDuty（用于通知/事件管理）。

通过利用 Keep Provider，用户可以深度集成 Keep 与他们使用和信赖的工具，提供一种灵活而强大的方式，在统一面板中轻松管理这些工具。
